...
//判断是否为同一直线
bool calculateGeometry::isTheSameLine(wayLine l1, wayLine l2)
{
...
QPair<double, double> calculateGeometry::farthestPointToLine(wayLine &sideLine)
{
...
//点到直线的距离
double calculateGeometry::calculatePointLineDistance(QPair<double, double> p, wayLine &l)
{
    return xyDistance(p, calculateVertialPoint(p, l));
}

//计算垂足坐标
QPair<double, double> calculateGeometry::calculateVertialPoint(QPair<double, double> point, wayLine &line)
{
...
//2平行线的距离
double calculateGeometry::calculateLinelineDistance(wayLine l1, wayLine l2)
{
    return(qFabs(l1.intercept() - l2.intercept())/qSqrt(1 + l1.slope() * l1.slope()));
}

//2直线的交点
QPair<double, double> calculateGeometry::calculateIntersectPoint(wayLine &l1, wayLine &l2,  QPair<double, double> ragePoint1, QPair<double, double> ragePoint2)
{
    //设2直线交点（x0, y0)
    double x0, y0;
