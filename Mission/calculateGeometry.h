...
public:
    ...
    ~calculateGeometry();
    bool isTheSameLine(wayLine l1, wayLine l2);
    //设置坐标系原点
    ...
    //求多边形的最远点(相对于参考边)
    QPair<double, double> farthestPointToLine(wayLine &sideLine);
    ...
    //点到直线的距离
    double calculatePointLineDistance(QPair<double, double> p, wayLine &l);
    //求垂足
    QPair<double, double> calculateVertialPoint(QPair<double, double> point, wayLine &line);

    //2平行线之间的距离
    double calculateLinelineDistance(wayLine l1, wayLine l2);
    //2直线的交点
    QPair<double, double> calculateIntersectPoint(wayLine &l1, wayLine &l2, QPair<double, double> ragePoint1, QPair<double, double> ragePoint2);
    //多边形的面积
