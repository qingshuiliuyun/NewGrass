...
void polygonField::setSideLines()
{
    for(int i = 0; i < m_cornerList.size(); i++){
        if(i ==(m_cornerList.size() - 1)){
            m_sideLineList.append(new wayLine(m_cornerXYList.at(i), m_cornerXYList.at(0)));
        } else {
            m_sideLineList.append( new wayLine(m_cornerXYList.at(i), m_cornerXYList.at(i + 1)));
        }  
...
QList<wayLine *> polygonField::sidelines()
{
